...
Необходимо разработать фреймворк, который будет подключаться к базе данных MySQL и
использовать SQL-запросы для работы с данными PrestaShop.
...
1. Написать тест на создание нового клиента (таблица `ps_customer`). Для этого подготовить данные нового клиента, 
создать клиента с помощью тестового фреймворка и проверить наличие клиента в базе сделав запрос по его `id`.

2. Написать тест на обновление данных существующего клиента. Нужно обновить значения полей `firstname`, `lastname` и `email`. 
После обновления - убедиться, что данные клиента обновились сделав запрос в БД.
